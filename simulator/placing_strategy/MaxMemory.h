...
struct MaxMemory : public IPlacingStrategy {
    unsigned BestMachineIndex(std::vector<const Machine*>& machines, const Task* /*task*/) override {
        ...
        unsigned maxMemory{machines[0]->availableMemory_};

        for (size_t i = 1; i < split; ++i) {
            const Machine* machine = machines[i];

            if (machine->availableMemory_ > maxMemory) {
                maxMemory = machine->availableMemory_;
